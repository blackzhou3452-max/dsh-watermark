...
export function apply(ctx, config = {}) {
  ...
  const disposeTool = tools.register({
    ...
    description: [
      ...
      'Modes: default heuristic detection from one image; `template` + `restore` inverts a '
      + 'known semi-transparent mark exactly (run the script once with --learn on a '
      + 'marked/clean pair of the same picture to obtain such a signature); `strategy: "multi"` '
      + 'uses >=3 frames that share one mark position. ',
      'Always check the reported changed% and confidence, and use dryRun + maskOutDir to '
      + 'inspect a mask before trusting a batch.',
    ].join(''),
    parameters: {
      type: 'object',
      ...
      properties: {
        paths: {
          type: 'array',
          items: { type: 'string' },
          description: 'Files or directories to process (directories are walked).',
        },
        search: {
          type: 'string',
          enum: ['all', 'bottom-right', 'bottom-left', 'top-right', 'top-left', 'bottom',
            'top', 'corners', 'none'],
          description: 'Where to look for the mark. Narrowing this is the cheapest way to '
            + 'avoid false positives on busy images. Default all.',
        },
        rect: {
          type: 'array',
          items: { type: 'string' },
        ...
        template: {
          type: 'string',
          description: 'Watermark PNG: a signature written by --learn (RGBA, placed via its '
            + '.json sidecar) or a black-on-white shape.',
        },
        restore: {
          type: 'boolean',
          description: 'Invert the overlay exactly: original = (observed - a*color)/(1 - a). '
            + 'Needs `template`. Opaque pixels cannot be recovered and are inpainted instead.',
        },
        strategy: {
          type: 'string',
          enum: ['auto', 'single', 'multi', 'template'],
          description: 'multi needs >=3 same-size frames that share one mark position.',
        },
        period: {
          type: 'string',
          description: '"auto" to estimate a tiled mark\'s lattice, or "W,H" to give it.',
        },
        color: { type: 'string', description: 'Mark colour B,G,R when the template has no colour.' },
        alpha: {
          type: 'number',
          description: 'Peak alpha for a binary (shapeless) template; default: calibrated '
        ...
        outExt: {
          type: 'string',
          description: 'Output extension, e.g. "png" (default: the input\'s). Use png for JPEG '
            + 'inputs: writing JPEG re-compresses every pixel of the picture, so the output '
            + 'would no longer be a record of what the tool changed.',
        },
        ...
        kSigma: {
          type: 'number',
          description: 'Noise sigmas the residual must beat (default 3). Lower it for a very '
            + 'faint mark, raise it on a noisy frame.',
        },
        contrastDelta: { type: 'number', description: 'Absolute residual floor (default 12).' },
        saliencyRatio: {
          type: 'number',
          description: 'How far above its own neighbourhood a pixel must be (default 1.8).',
        },
        minSaliency: { type: 'number', description: 'Component-level saliency gate (default 5).' },
        minBlob: { type: 'number', description: 'Drop components smaller than this (default 40).' },
        window: { type: 'number', description: 'Base background window radius (default 31).' },
        ...
        multiRatio: { type: 'number', description: 'multi strategy: spread ratio (default 0.55).' },
        maxAreaFrac: {
          type: 'number',
          description: 'Reject a component bigger than this share of the search area (0.25).',
        },
      },
    },
